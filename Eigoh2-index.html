...
<head>
    ...
    <style>
        ...
        /* --- 単語帳のスタイル --- */
        #wordbookModal { background-color: rgba(0,0,0,0.9); flex-direction: column; justify-content: flex-start; padding: 20px; box-sizing: border-box; }
        .wordbook-container { width: 100%; max-width: 600px; height: calc(100% - 100px); overflow-y: auto; text-align: left; padding: 15px; border: 4px solid #fff; }
        .wordbook-level-header { font-family: 'Press Start 2P', cursive; color: #87cefa; font-size: 1.8rem; margin: 20px 0 10px 0; text-align: center; }
        .word-entry { margin: 8px 0; font-size: 1.2rem; }
        #wordbook-close-button { margin-top: 20px; max-width: 400px; }
...
<body>
    <!-- Audio Elements -->
    <audio id="audio-title" src="http://googleusercontent.com/file_content/3" loop preload="auto"></audio>
    <audio id="audio-battle" src="http://googleusercontent.com/file_content/2" loop preload="auto"></audio>
    <audio id="audio-boss" src="http://googleusercontent.com/file_content/1" loop preload="auto"></audio>
    <audio id="sfx-correct" src="http://googleusercontent.com/file_content/4" preload="auto"></audio>
    <audio id="sfx-incorrect" src="http://googleusercontent.com/file_content/6" preload="auto"></audio>
    <audio id="sfx-levelup" src="http://googleusercontent.com/file_content/5" preload="auto"></audio>
    ...
    <div id="wordbookModal" class="screen hidden">
        <div class="wordbook-container">
             <h2 class="wordbook-level-header">--- LEVEL 1 ---</h2>
            <div id="wordlist-level1" class="wordlist"></div>
            <h2 class="wordbook-level-header">--- LEVEL 2 ---</h2>
            <div id="wordlist-level2" class="wordlist"></div>
            <h2 class="wordbook-level-header">--- LEVEL 3 ---</h2>
            <div id="wordlist-level3" class="wordlist"></div>
        </div>
        <button id="wordbook-close-button" class="close-button">閉じる</button>
    </div>
    ...
    <script>
        ...
        const dom = {
            ...
            wordbook: {
                modal: document.getElementById('wordbookModal'),
                closeButton: document.getElementById('wordbook-close-button'),
                level1: document.getElementById('wordlist-level1'),
                level2: document.getElementById('wordlist-level2'),
                level3: document.getElementById('wordlist-level3'),
            },
            audio: {
        ...
        function playBGM(track) {
            ...
            if (currentBGM === track && !track.paused) return;
            if (currentBGM) {
                currentBGM.pause();
            }
            track.currentTime = 0;
            track.play().catch(e => console.error("BGM play failed:", e));
            currentBGM = track;
        }
        ...
        function spawnEnemy() {
            ...
            if (isBossTime) {
                playBGM(dom.audio.boss);
            } else {
                playBGM(dom.audio.battle);
            }
        ...
        dom.quiz.grid.addEventListener('click', async e => {
            ...
            if (isCorrect) { 
                playSFX(dom.audio.sfxCorrect);
                e.target.classList.add('correct'); 
            } else { 
                playSFX(dom.audio.sfxIncorrect);
                e.target.classList.add('incorrect'); 
                dom.quiz.buttons.forEach(b => { if (b.textContent === currentQuestion.meaning) b.classList.add('correct-answer'); }); 
            }
        ...
        function gameOver() { 
            stopAllBGM();
            playBGM(dom.audio.title);
            showBattleMessage(`GAME OVER... Final Score: ${score}`, () => { 
        ...
        function populateWordbook() {
            [1, 2, 3].forEach(level => {
                const container = dom.wordbook[`level${level}`];
                container.innerHTML = '';
                const wordsForLevel = wordData.filter(w => w.level === level);
                wordsForLevel.forEach(word => {
                    const p = document.createElement('p');
                    p.className = 'word-entry';
                    p.textContent = `${word.word}: ${word.meaning}`;
        ...
        document.getElementById('bookButton').addEventListener('click', () => {
            initAudio();
            dom.titleScreen.classList.add('hidden');
            dom.wordbook.modal.classList.remove('hidden');
        });
        dom.wordbook.closeButton.addEventListener('click', () => {
            dom.wordbook.modal.classList.add('hidden');
            dom.titleScreen.classList.remove('hidden');
        });
        ...
        document.getElementById('quit-yes-button').addEventListener('click', () => { 
            togglePause(false); 
            stopAllBGM();
            playBGM(dom.audio.title);
            dom.battleScreen.classList.add('hidden'); 
